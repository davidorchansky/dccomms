...
static int
getOptions(int argc, char *argv[], struct debtEncParam *e, struct debtDecParam *d, char** imId, int* imIdSize, 	videoTransmissionConfig* config, int * Nf)
{
	int opt;
	int error = 0;
	int xi = 0;
	...
	while ((opt = getopt(argc, argv, "v:db:a:c:et:n:k:r:q:u:x:p:m:s:l:i:y:gf:h:I:W:H:F:L:D:A:N:")) != -1) {
		switch (opt) {
		case 'N':
			*Nf = atoi(optarg);
			break;
...
int main(int argc, char ** argv) {
	...
	int act = getOptions(argc, argv, &e, &d, &imId, &bIdLength, &config, & TEST_NUM_FRAMES);
					...
					if(read(fd,inputBuffer,yuyv422_size)>0)
					{
						...
						if(newestFrame == 2)  
						{
							...
							if(res)
							{
								...
								if(imagenesEnviadas == TEST_NUM_FRAMES)
								{
