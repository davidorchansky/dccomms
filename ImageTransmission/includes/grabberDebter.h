...
int init_mmap(int fd)
{
    ...
    buffer_G = (uint8_t*) mmap (NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, buf.m.offset);
...
read_img(int *size, FILE *f)
{
	...
	unsigned char *cimg = (uint8_t*)malloc(rsize);
	if (cimg) {
...
graph(struct debtEncParam *e, struct debtDecParam *d)
{
	...
	if (src) {
		...
		int fixed = 0;
		if (e->maxsize) {
			dbtbuf = (uint8_t*)malloc(dbtbuflen = e->maxsize);
			fixed = 1;
		}
		/* encode it! */
