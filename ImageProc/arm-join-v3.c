...
static void copiarArray_Uint8_Double(double* dst, uint8_t* src, double* dst, unsigned int size)
{
	uint8_t * sptr;
	double *  dptr;
...
void showFilter(FILE * s, double * filter, int size)
{
	fprintf(s, "[ ");
	int i;
	for(i=0; i<size; i++)
...
void showFilterM(FILE * s, double ** filter, int height, int width)
{
	fprintf(s, "[ ");
	int f,c;
...
double gaussianZeroMean2D(double x, double y, double sigma)
{
	double res = pow(M_E, (-1./2)*(x*x+y*y)/(sigma*sigma));

	return res;
}

void getGaussian2DFilter(double ** filtro, unsigned int tam, double sigma)
{
	int li = -(tam >> 1);
	int ls = li*(-1);
	double v, suma=0;
...
void getGaussianFilter(double * filtro, unsigned int tam, double sigma)
{
	int offset = tam >> 1;
	int idx;
	double * ptr;
...
void _aplicarFiltro(double * dst, unsigned int width, unsigned int height, unsigned int size, double * hfiltro, double * vfiltro, unsigned int tamFiltro)
{
	...
	double * posIniOrigBuf = origBuf + offsetInicial;
	double * posIniDestBuf = destBuf + offsetInicial;

	double * sptr, *dptr;
	double * maxf = posIniDestBuf + width * filas;
	double * centroFiltro = hfiltro + foffset;

	for(sptr = posIniOrigBuf, dptr = posIniDestBuf; dptr < maxf; sptr+=ioffset, dptr+=ioffset)
	{
		double * maxc;
		for(maxc = dptr + columnas; dptr < maxc; dptr +=1, sptr+=1)
		{
			...
			*dptr = 0;
			...
			{
				*dptr += *(sptr+idx) * *(centroFiltro+idx);
			}
		}
	}
	...
	posIniOrigBuf = origBuf + offsetInicial;
	posIniDestBuf = destBuf + offsetInicial;

	double * maxc = posIniDestBuf + columnas;
	double * dptrc, *sptrc;

	for(sptr = posIniOrigBuf, sptrc = posIniOrigBuf, dptr = posIniDestBuf, dptrc = posIniDestBuf; dptr < maxc; sptr = sptrc+1, sptrc = sptr, dptr = dptrc+1, dptrc = dptr)
	{
		for(maxf = dptr + filas * width; dptr < maxf; dptr += width, sptr += width)
		{
			...
			*dptr = 0;
			for(idx = -foffset; idx <= foffset; idx++)
			{
				*dptr += *(sptr+idx*(int)width) * *(centroFiltro+idx);
			}
		}
	}
...
void _aplicarFiltro_size3(double * dst, unsigned int width, unsigned int height, unsigned int size, double * hfiltro, double * vfiltro)
{
	double * bufferDeTrabajo = (double *) malloc(size * sizeof(double));
	double * origBuf = dst;
	double * destBuf = bufferDeTrabajo;
	...
	*fp2 = centroFiltro+1;
	...
	for(sptr = posIniOrigBuf, sptrc = posIniOrigBuf, dptr = posIniDestBuf, dptrc = posIniDestBuf; dptr < maxc; sptr = sptrc+1, sptrc = sptr, dptr = dptrc+1, dptrc = dptr)
	{
		for(maxf = dptr + filas * width; dptr < maxf; dptr += width, sptr += width)
		{
			//Aplicamos el filtro verticalmente
			*dptr = 0;
			*dptr += *(sptr - (int)width)**fp0;
			*dptr += *(sptr)**fp1;
			*dptr += *(sptr + (int)width)**fp2;
		}
	}
	free(bufferDeTrabajo);
...
void _aplicarFiltro_staticSize(double * dst, unsigned int width, unsigned int height, unsigned int size, double * hfiltro, double * vfiltro)
{
	double * bufferDeTrabajo = (double *) malloc(size * sizeof(double));
	double * origBuf = dst;
	double * destBuf = bufferDeTrabajo;
	...
	*fp6 = centroFiltro+3;
	...
	for(sptr = posIniOrigBuf, sptrc = posIniOrigBuf, dptr = posIniDestBuf, dptrc = posIniDestBuf; dptr < maxc; sptr = sptrc+1, sptrc = sptr, dptr = dptrc+1, dptrc = dptr)
	{
		for(maxf = dptr + filas * width; dptr < maxf; dptr += width, sptr += width)
		{
			...
			*dptr += *(sptr + (int)width)**fp2;
			#elif FS_5
			*dptr += *(sptr-2*(int)width)**fp0;
			*dptr += *(sptr-(int)width)**fp1;
			*dptr += *(sptr)**fp2;
			*dptr += *(sptr+(int)width)**fp3;
			*dptr += *(sptr+2*(int)width)**fp4;
			#else
			*dptr += *(sptr-3*(int)width)**fp0;
			*dptr += *(sptr-2*(int)width)**fp1;
			*dptr += *(sptr-(int)width)**fp2;
			*dptr += *(sptr)**fp3;
			*dptr += *(sptr+(int)width)**fp4;
			*dptr += *(sptr+2*(int)width)**fp5;
			*dptr += *(sptr+3*(int)width)**fp6;
			#endif
...
void aplicarFiltro_Double_Double(double * src, double * dst, unsigned int width, unsigned int height, double * hfiltro, double * vfiltro, unsigned int tamFiltro)
{
	unsigned int size = height * width;

	memcpy(dst, src, sizeof(double)*size);

	_aplicarFiltro(dst, width, height, size, hfiltro, vfiltro, tamFiltro);
}

void aplicarFiltro_Double_Double_staticSize(double * src, double * dst, unsigned int width, unsigned int height, double * hfiltro, double * vfiltro)
{
	unsigned int size = height * width;

	memcpy(dst, src, size * sizeof(double));

	_aplicarFiltro_staticSize(dst, width, height, size, hfiltro, vfiltro);

}

void aplicarFiltro_Uint8_Double(uint8_t * src, double * dst, unsigned int width, unsigned int height, double * hfiltro, double * vfiltro, unsigned int tamFiltro)
{
	unsigned int size = height * width;
	...
	for(_sptr = src, _dptr = dst; _sptr < maxsptr; _sptr++, _dptr++)
	{
		*_dptr = (double) *_sptr;
	}
...
void aplicarFiltro_Uint8_Double_staticSize(uint8_t * src, double * dst, unsigned int width, unsigned int height, double * hfiltro, double * vfiltro)
{
	unsigned int size = height * width;
...
static void aplicarFiltro_noSeparable_Uint8_Double_staticSize(uint8_t * src, double * dst, unsigned int width, unsigned int height, double ** filtro)
{
	...
	copiarArray_Uint8_Double(dst, src, size);
	#ifdef FS_3
	unsigned int foffset = 1;
	#elif FS_5
	unsigned int foffset = 2;
	#else
	unsigned int foffset = 3;
	#endif
	...
	for(f=foffset; f < maxHeight; f++)
	{
		for(c=foffset; c < maxWidth; c++)
		{
			...
			*cpixel += sM[f4][c4] * filtro[4][4];
			...
			#else
			int c0=c-3, c1=c-2, c2=c-1, c3=c, c4=c+1, c5=c+2, c6=c+3,
			f0=f-3, f1=f-2, f2=f-1, f3=f, f4=f+1, f5=c+2, f6=c+3;
			...
			*cpixel += sM[f0][c3] * filtro[0][3];
			*cpixel += sM[f0][c4] * filtro[0][4];
			*cpixel += sM[f0][c5] * filtro[0][5];
			*cpixel += sM[f0][c6] * filtro[0][6];
			...
			*cpixel += sM[f1][c3] * filtro[1][3];
			*cpixel += sM[f1][c4] * filtro[1][4];
			*cpixel += sM[f1][c5] * filtro[1][5];
			*cpixel += sM[f1][c6] * filtro[1][6];
			...
			*cpixel += sM[f2][c3] * filtro[2][3];
			*cpixel += sM[f2][c4] * filtro[2][4];
			*cpixel += sM[f2][c5] * filtro[2][5];
			*cpixel += sM[f2][c6] * filtro[2][6];
			...
			*cpixel += sM[f3][c3] * filtro[3][3];
			*cpixel += sM[f3][c4] * filtro[3][4];
			*cpixel += sM[f3][c5] * filtro[3][5];
			*cpixel += sM[f3][c6] * filtro[3][6];
			...
			*cpixel += sM[f4][c3] * filtro[4][3];
			*cpixel += sM[f4][c4] * filtro[4][4];
			*cpixel += sM[f4][c5] * filtro[4][5];
			*cpixel += sM[f4][c6] * filtro[4][6];

			*cpixel += sM[f5][c0] * filtro[5][0];
			*cpixel += sM[f5][c1] * filtro[5][1];
			*cpixel += sM[f5][c2] * filtro[5][2];
			*cpixel += sM[f5][c3] * filtro[5][3];
			*cpixel += sM[f5][c4] * filtro[5][4];
			*cpixel += sM[f5][c5] * filtro[5][5];
			*cpixel += sM[f5][c6] * filtro[5][6];

			*cpixel += sM[f6][c0] * filtro[6][0];
			*cpixel += sM[f6][c1] * filtro[6][1];
			*cpixel += sM[f6][c2] * filtro[6][2];
			*cpixel += sM[f6][c3] * filtro[6][3];
			*cpixel += sM[f6][c4] * filtro[6][4];
			*cpixel += sM[f6][c5] * filtro[6][5];
			*cpixel += sM[f6][c6] * filtro[6][6];
			#endif
...
static void aplicarFiltro_noSeparable_Double_Double_staticSize(double * src, double * dst, unsigned int width, unsigned int height, double ** filtro)
{

	unsigned int size = width * height;
	double ** sM = (double**) malloc(height * sizeof(double*));
	double ** dM = (double**) malloc(height * sizeof(double*));
...
static void aplicarFiltro_noSeparable_Double_Double_size3(double * src, double * dst, unsigned int width, unsigned int height, double ** filtro)
{

	unsigned int size = width * height;
	double ** sM = (double**) malloc(height * sizeof(uint8_t*));
	double ** dM = (double**) malloc(height * sizeof(double*));
...
static void aplicarFiltro_noSeparable_Double_Double(double * src, double * dst, unsigned int width, unsigned int height, double ** filtro, unsigned int tfiltro)
{
...
static void aplicarFiltro_noSeparable_Uint8_Double(uint8_t * src, double * dst, unsigned int width, unsigned int height, double ** filtro, unsigned int tfiltro)
{
...
static void computeGradientX_Double_Double(double * src, double * dst, unsigned int width, unsigned int height)
{
	double sfxa[3] = {-1,0,1};
	double sfxb[3] = {1,2,1};
...
static void computeGradientY_noSeparable_Double_Double(double * src, double * dst, unsigned int width, unsigned int height)
{
	unsigned int size = width * height;
/*	
	double Gy[3][3] = 
...
static void escalar_Double_Uint8(double * ygradiente, uint8_t * ygescalado, unsigned int size)
{
	double * sptr,
...
static void saveImage_Double(int fd, uint8_t * header, unsigned int hlength, double * content, unsigned int clength)
{
	int writen = write(fd, header, hlength);
	uint8_t * scaled = (uint8_t*) malloc(clength);

	escalar_Double_Uint8(content, scaled, clength);
...
int main(int argc, char ** argv)
{
	...
	filtroM = (double**) malloc(tamFiltro*sizeof(double*));
	filtro = (double*) malloc(tamFiltro*tamFiltro * sizeof(double));

	for(f = 0, gptr = filtro; f < tamFiltro; f++, gptr += tamFiltro)
	{
		filtroM[f] = gptr;
	}

	double * filtro1d = (double*) malloc(tamFiltro * sizeof(double));
	getGaussianFilter(filtro1d, tamFiltro, sigma);
	getGaussian2DFilter(filtroM, tamFiltro, sigma);
	showFilterM(stderr, filtroM, tamFiltro, tamFiltro);

	showFilter(stderr, filtro1d, tamFiltro);
#ifdef DEBUG
	...
	if(read_ppmHeader(&width, &height) == 0)
	{
		...
		double * gsFiltrado = (double*) malloc(pixelLength * sizeof(double));
		...
		#ifdef FS_3
		aplicarFiltro_noSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#elif FS_5
		aplicarFiltro_noSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#elif FS_7
		aplicarFiltro_noSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#else
		aplicarFiltro_noSeparable_Uint8_Double(gs, gsFiltrado, width, height, filtroM, tamFiltro);
		#endif

		#else

		#ifdef FS_3
		aplicarFiltro_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtro1d, filtro1d);
		#elif FS_5
		aplicarFiltro_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtro1d, filtro1d);
		#elif FS_7
		aplicarFiltro_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtro1d, filtro1d);
		#else
		aplicarFiltro_Uint8_Double(gs, gsFiltrado, width, height, filtro1d, filtro1d, tamFiltro);
		#endif
		...
		escalar_Double_Uint8(xgradiente, xgescalado, pixelLength);
		escalar_Double_Uint8(ygradiente, ygescalado, pixelLength);
		escalar_Double_Uint8(mgradiente, mgescalado, pixelLength);
		escalar_Double_Uint8(dgradiente, dgescalado, pixelLength);
		escalar_Uint8_Uint8(dgdiscreta, dgdescalado, pixelLength);
		escalar_Double_Uint8(mgthin, mgthinescalado, pixelLength);
		...
		saveImage_Double(ffiltrado, pgm, pgmhl, gsFiltrado, pixelLength);
		free(gsFiltrado);
