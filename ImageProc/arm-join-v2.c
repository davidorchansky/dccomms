	...
	*fp6 = centroFiltro+3;
	...
	for(dptr = posicionInicial; dptr < maxf; dptr+=ioffset)
	{
		...
		for(maxc = dptr + columnas; dptr < maxc; dptr +=1)
		{
			...
			double valorFinal = 0;
			#ifdef FS_3
			valorFinal += *(dptr-1)**fp0;
			valorFinal += *(dptr)**fp1;
			valorFinal += *(dptr+1)**fp2;
			#elif FS_5
			valorFinal += *(dptr-2)**fp0;
			valorFinal += *(dptr-1)**fp1;
			valorFinal += *(dptr)**fp2;
			valorFinal += *(dptr+1)**fp3;
			valorFinal += *(dptr+2)**fp4;
			#else
			valorFinal += *(dptr-3)**fp0;
			valorFinal += *(dptr-2)**fp1;
			valorFinal += *(dptr-1)**fp2;
			valorFinal += *(dptr)**fp3;
			valorFinal += *(dptr+1)**fp4;
			valorFinal += *(dptr+2)**fp5;
			valorFinal += *(dptr+3)**fp6;
			#endif
	...
	for(dptr = posicionInicial, dptrc = posicionInicial; dptr < maxc; dptr = dptrc+1, dptrc = dptr)
	{
		for(maxf = dptr + filas * width; dptr < maxf; dptr += width)
		{
			...
			double valorFinal = 0;
			#ifdef FS_3
			valorFinal += *(dptr - (int)width)**fp0;
			valorFinal += *(dptr)**fp1;
			valorFinal += *(dptr + (int)width)**fp2;
			#elif FS_5
			valorFinal += *(dptr-2*(int)width)**fp0;
			valorFinal += *(dptr-(int)width)**fp1;
			valorFinal += *(dptr)**fp2;
			valorFinal += *(dptr+(int)width)**fp3;
			valorFinal += *(dptr+2*(int)width)**fp4;
			#else
			valorFinal += *(dptr-3*(int)width)**fp0;
			valorFinal += *(dptr-2*(int)width)**fp1;
			valorFinal += *(dptr-(int)width)**fp2;
			valorFinal += *(dptr)**fp3;
			valorFinal += *(dptr+(int)width)**fp4;
			valorFinal += *(dptr+2*(int)width)**fp5;
			valorFinal += *(dptr+3*(int)width)**fp6;
			#endif
...
void aplicarFiltro_Uint8_Uint8_ConPrecision(uint8_t * src, uint8_t * dst, unsigned int width, unsigned int height, double * hfiltro, double * vfiltro, unsigned int tamFiltro)
{
	unsigned int size = height * width;
