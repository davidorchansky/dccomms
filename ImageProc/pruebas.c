...
/*
	...
	double * _src = (double*) malloc(size * sizeof(double));

	for(sptr = src, dptr = _src; sptr < maxsptr; sptr++, dptr++)
	{
		*dptr = (double) *sptr;
	}
	aplicarFiltro_Double_Double(_src, dst, width, height, hfiltro, vfiltro, tamFiltro);
	free(_src);
*/
}
...
int main(int argc, char ** argv)
{
	...
	if(read_ppmHeader(&width, &height) == 0)
	{
		...
		#ifdef FS_3
		aplicarFiltroNoSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#elif FS_5
		aplicarFiltroNoSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#elif FS_7
		aplicarFiltroNoSeparable_Uint8_Double_staticSize(gs, gsFiltrado, width, height, filtroM);
		#else
		aplicarFiltroNoSeparable_Uint8_Double(gs, gsFiltrado, width, height, filtroM, tamFiltro);
		#endif
		...
		aplicarFiltro_Double_Double(gsFiltrado, xgradiente, width, height, sfxa, sfxb, 3);
		//aplicarFiltroGradienteSeparable_Double_Double_staticSize(gsFiltrado, xgradiente, width, height, sfxa, sfxb);
		//aplicarFiltroGradiente_Double_Double(gsFiltrado, xgradiente, width, height, Gx);
#ifdef TIMMING
		gettimeofday(&t1, NULL);
		...
		aplicarFiltro_Double_Double(gsFiltrado, ygradiente, width, height, sfya, sfyb, 3);
		//aplicarFiltroGradienteSeparable_Double_Double_staticSize(gsFiltrado, ygradiente, width, height, sfya, sfyb);
		aplicarFiltroGradiente_Double_Double(gsFiltrado, ygradiente, width, height, Gy);
