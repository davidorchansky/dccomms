...
ICommsLink &IStreamCommsDevice::operator<<(const DataLinkFramePtr &dlf) {
  if (dlf->_dataIn) {
    ...
    // FlushIO();//Lo ideal seria FlushOutput, pero en algun lado hay algo que
    // hace que se llene el buffer de entrada
    // y al final llega a bloquearse la comunicación... (TODO: comprobar qué es
    // lo que hace que se llene el buffer de entrada)
  }
  return *this;
}
}
